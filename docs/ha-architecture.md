...
- **TLS:** cert-manager + Let's Encrypt DNS-01. `cloudless.gr` uses the Route 53 solver (zone Z079608614L53CC4EAZM3). `cloudless.online` uses the Cloudflare solver — `cloudless.online` migrated to Cloudflare DNS and Route 53 is no longer authoritative. Both solvers are configured in `k3s/cert-manager/cluster-issuer.yaml`. Prerequisite: provision the `cloudflare-api-token` Secret in cert-manager namespace before the current `cloudless.online` cert expires (see `cloudflare-api-token.example.yaml`).
