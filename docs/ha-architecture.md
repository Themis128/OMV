...
- **TLS:** cert-manager + Let's Encrypt DNS-01 via Route 53 for `cloudless.gr`. Works behind NAT/CGNAT because no inbound port 80 challenge is needed. `cloudless.online` is now Cloudflare-terminated — its in-cluster cert and the DNS-01 solver (which assumed a Route 53 zone) need re-verifying against the Cloudflare migration.
