...
CronJobs. Routes:
  ...
  GET  /healthz       → 200 OK (kubelet probes; in-cluster only)
...
This service holds no state. Drift detection + rollout logic live in the
image-sync / config-sync CronJobs — we just *trigger* them via Job creation.
Replay-bombing is bounded by Job RBAC and kubectl rate limits; even a flood
produces no-op Jobs that exit immediately when in sync.
"""
